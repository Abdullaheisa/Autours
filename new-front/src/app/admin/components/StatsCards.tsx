...
export default function StatsCards() {
  ...
  const avgRating      = rawData?.avgRating ?? null;
  ...
  const cards = [
    ...
    },
    {
      ...
      value:       avgRating !== null ? String(avgRating) : "N/A",
      icon:        Star,
      bgColor:     "bg-amber-50",
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6">
      {cards.map((stat, index) => (
        <div
          key={index}
          className={`bg-white rounded-2xl p-3 sm:p-4 border ${stat.borderColor} shadow-sm hover:shadow-md transition-shadow flex items-center gap-3 sm:gap-4`}
        >
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-lg sm:text-xl lg:text-2xl font-black text-gray-900 mt-0.5 truncate">
              {stat.value}
            </p>
          </div>
        </div>
