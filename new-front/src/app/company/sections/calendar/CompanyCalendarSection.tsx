...
// Generate Mock Data scoped to a single company (as robust fallback)
...
export default function CompanyCalendarSection() {
  ...
  useEffect(() => {
    setIsLoading(true);
    supplierApi.getRentals().then((res: any) => {
      ...
      console.warn("Failed to fetch supplier bookings, using fallback mock data:", err.message);
  ...
  const handleDownload = () => {
    const headers = ["Booking ID", "Vehicle", "Category", "Grade", "Duration", "Status", "Date"];
    ...
    const fileName = `My_Bookings_${format(selectedDate, "yyyy_MM_dd")}.csv`;
    ...
    for (let i = 1; i <= 5; i++) {
      ...
      if (bookings.length > 0) {
        result.push({
          ...
          summary: `${bookings.length} upcoming ${bookings.length === 1 ? 'booking' : 'bookings'}`
        });
      }
    }
    return result;
