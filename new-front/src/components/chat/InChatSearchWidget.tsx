...
const POPULAR_COUNTRIES = [
  { label: '🇹🇷 تركيا', en: 'Turkey', flag: '🇹🇷' },
  { label: '🇦🇪 الإمارات', en: 'United Arab Emirates', flag: '🇦🇪' },
  { label: '🇪🇬 مصر', en: 'Egypt', flag: '🇪🇬' },
  { label: '🇬🇪 جورجيا', en: 'Georgia', flag: '🇬🇪' },
  { label: '🇪🇸 إسبانيا', en: 'Spain', flag: '🇪🇸' },
  { label: '🇧🇭 البحرين', en: 'Bahrain', flag: '🇧🇭' },
  { label: '🇯🇴 الأردن', en: 'Jordan', flag: '🇯🇴' },
  { label: '🇰🇼 الكويت', en: 'Kuwait', flag: '🇰🇼' },
];

function formatDisplayDate(dateStr: string, isEnglish: boolean) {
  if (!dateStr) return '';
  const d = new Date(dateStr);
  if (isNaN(d.getTime())) return dateStr;
  return d.toLocaleDateString(isEnglish ? 'en-US' : 'ar-EG', {
    day: 'numeric',
    month: 'short',
  });
}
...
export default function InChatSearchWidget({
  ...
  // 1. Fetch available live locations from backend
  useEffect(() => {
    let isMounted = true;
  ...
  // 2. Pre-fill initial location from customer message (e.g. "Turkey" -> Pre-select Turkey)
  useEffect(() => {
    if (!initialLocation) return;
    
    // Check universal map first (e.g. "تركيا" -> "Turkey", "دبي" -> "Dubai")
    ...
    if (locations.length > 0) {
      // Find matching branch or country
  ...
  const handleSelectCountryPreset = (countryEn: string, countryAr: string) => {
    ...
    if (countryBranch) {
      ...
      setLocationSearchInput(isEnglish ? countryEn : countryAr);
    }
  };

  const filteredLocations = locations.filter((loc) => {
    const q = locationSearchInput.toLowerCase().trim();
    const matchesCountry = !selectedCountryFilter || (loc.country || '').toLowerCase() === selectedCountryFilter.toLowerCase();
    ...
    return matchesCountry && (display.includes(q) || name.includes(q) || city.includes(q) || country.includes(q));
  });
  ...
  return (
    <div className="w-full bg-white border-2 border-amber-400/90 rounded-2xl p-3.5 sm:p-4 shadow-xl shadow-amber-500/10 text-gray-900 transition-all duration-300" dir={isEnglish ? 'ltr' : 'rtl'}>
      {/* Header */}
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h4 className="text-[13px] font-black text-gray-950 leading-tight">
              {isEnglish ? 'Select Destination & Dates' : 'حدد وجهتك وتواريخ الإيجار'}
            </h4>
            <p className="text-[10.5px] text-gray-500 font-medium">
              {isEnglish ? 'Choose location & duration to view live prices' : 'اختر المدينة والتواريخ لعرض السيارات المتاحة فوراً'}
            </p>
          </div>
        </div>
      ...
      <form onSubmit={handleSearchSubmit} className="space-y-3">
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          >
            <div className="flex items-center gap-2 flex-1 min-w-0">
              ...
              <input
                ...
                placeholder={isEnglish ? 'Search airport, city, or country...' : 'ابحث عن مطار، مدينة، أو دولة...'}
          ...
          {showLocationDropdown && (
            <div className="absolute top-full left-0 right-0 mt-1.5 bg-white border border-gray-200 rounded-xl shadow-xl z-50 p-2 max-h-60 overflow-y-auto space-y-2">
              ...
              <div className="flex items-center gap-1 overflow-x-auto pb-1.5 border-b border-gray-100 scrollbar-none">
                <button
                  ...
                  className={`text-[10px] px-2.5 py-1 rounded-lg font-bold whitespace-nowrap transition-colors ${
                ...
                {POPULAR_COUNTRIES.map((c) => (
                  <button
                    key={c.en}
                    type="button"
                    onClick={() => handleSelectCountryPreset(c.en, c.label)}
                    className={`text-[10px] px-2.5 py-1 rounded-lg font-bold whitespace-nowrap flex items-center gap-1 transition-colors ${
                      selectedCountryFilter === c.en
                        ? 'bg-amber-500 text-white shadow-2xs'
                        : 'bg-gray-100 text-gray-700 hover:bg-amber-100'
                    }`}
                  >
                    <span>{c.flag}</span>
                    <span>{isEnglish ? c.en : c.label.replace(c.flag, '').trim()}</span>
                  </button>
                ))}
              </div>
              ...
              <div className="space-y-1">
                {filteredLocations.length > 0 ? (
                  ...
                  <p className="text-center py-3 text-xs text-gray-500 font-medium">
                    {isEnglish ? 'No matching locations found' : 'لم يتم العثور على فروع مطابقة'}
                  </p>
                )}
              </div>
            </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <div className="bg-gray-50/90 border border-gray-200 rounded-xl p-2 space-y-1.5 shadow-2xs">
            <label className="block text-[10px] font-bold text-gray-700 flex items-center gap-1">
              <Calendar className="w-3 h-3 text-amber-600 shrink-0" />
              <span>{isEnglish ? 'Pick-up Date' : 'تاريخ الاستلام'}</span>
            </label>
            <input
              type="date"
              value={dateFrom}
          ...
          <div className="bg-gray-50/90 border border-gray-200 rounded-xl p-2 space-y-1.5 shadow-2xs">
            <label className="block text-[10px] font-bold text-gray-700 flex items-center gap-1">
              <Calendar className="w-3 h-3 text-amber-600 shrink-0" />
              <span>{isEnglish ? 'Return Date' : 'تاريخ التسليم'}</span>
            </label>
            <input
              type="date"
              value={dateTo}
            ...
            {[3, 5, 7, 10, 14].map((days) => {
              ...
              return (
                <button
                  ...
                  className={`text-[10.5px] px-2.5 py-1 rounded-lg font-bold shrink-0 transition-all ${
                    isActive
        ...
        <div className="pt-1 flex flex-col gap-2">
          <div className="flex items-center justify-between text-[11px] bg-amber-50/70 px-3 py-1.5 rounded-xl border border-amber-200/80 gap-2">
            <div className="flex items-center gap-1.5 font-bold text-gray-900 truncate">
              <span className="bg-amber-400/20 text-amber-950 px-2 py-0.5 rounded-md text-[10.5px] font-black shrink-0">
                {calculatedDays} {isEnglish ? 'Days' : 'أيام إيجار'}
              </span>
              <span className="text-gray-600 text-[10.5px] truncate">
                {formatDisplayDate(dateFrom, isEnglish)} - {formatDisplayDate(dateTo, isEnglish)}
              </span>
            </div>
            <div className="text-emerald-700 font-extrabold flex items-center gap-1 text-[10.5px] bg-emerald-100/50 px-2 py-0.5 rounded-md shrink-0">
              ...
              <span>{isEnglish ? 'Free Cancel' : 'إلغاء مجاني'}</span>
            </div>
          </div>
          ...
          >
            ...
            <span>{isEnglish ? 'Find Available Cars Now' : 'ابحث عن السيارات المتاحة الآن'}</span>
          </button>
        </div>
      </form>
    </div>
