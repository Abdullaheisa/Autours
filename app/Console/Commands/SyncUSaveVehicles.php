...
class SyncUSaveVehicles extends AbstractVehicleSyncCommand
{
    use ResolvesLocalVehiclePhoto, NormalizesVehicleNames;

    protected $signature = 'usave:sync-vehicles
    ...
    protected $description = 'Sync vehicles from U-Save API into Autours.';
    ...
    protected function fetchVehiclesForDuration($service, Branch $branch, ?int $days, bool $hasCustomDates): array
    {
        ...
        try {
            ...
            Log::warning("U-Save sync warning for branch {$branch->name} (ID: {$branch->station_id}): " . $e->getMessage());
            return [];
        }
    }
}
