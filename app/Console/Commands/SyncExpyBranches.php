...
class SyncExpyBranches extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'expy:sync-branches {--dry-run : Only fetch from API and print what would be done} {--real : Perform the actual synchronization to the database}';
     ...
     * @var string
     */
    protected $description = 'Sync Expy locations as branches.';
    ...
    public function handle(): int
    {
        ...
        // ------------------------------------------------------------------
        // 2. Fetch all locations from Expy
        // ------------------------------------------------------------------
        $this->info('Fetching locations from Expy...');
        $locations = $service->getLocations();

        if (empty($locations)) {
            $this->error('No locations returned from Expy API.');
            return self::FAILURE;
        }
        ...
        // ------------------------------------------------------------------
        // 4. Delete branches no longer returned by Expy
        // ------------------------------------------------------------------
        ...
        $this->newLine();
        $this->info('========== Expy Branch Sync Complete ==========');
