...
class SyncNissaVehicles extends Command
{
     ...
     * @var string
     */
    protected $signature = 'nissa:sync-vehicles
     ...
     * @var string
     */
    protected $description = 'Sync vehicles from Nissa Car Rental JSON API into Autours. Creates one vehicle record per branch with per-station prices.';
    ...
    public function handle(): int
    {
        ...
        if ($allBranches->isEmpty()) {
            $this->warn('No Nissa branches found. Run: php artisan nissa:sync-branches');
            return self::FAILURE;
        }
        ...
        // ------------------------------------------------------------------
        // 4. Clean up orphaned Nissa images
        // ------------------------------------------------------------------
        $this->info('Cleaning up orphaned Nissa images...');
        $activeNissaPhotos = Vehicle::where('description', 'LIKE', '%[Nissa-GROUP-ID:%')
            ->whereNotNull('photo')
            ->pluck('photo')
            ->unique()
            ->values()
        ...
        $cleaned = 0;
        if (is_dir($imgDir)) {
            foreach (glob($imgDir . '/nissa_*') as $filePath) {
                $fileName = basename($filePath);
                if (! in_array($fileName, $activeNissaPhotos, true)) {
                    @unlink($filePath);
                    $cleaned++;
                }
            }
        ...
        // 4. Fetch all groups
        // ------------------------------------------------------------------
        $this->info('Fetching vehicle groups from Nissa...');
        $groups = $service->getGroups();

        if (empty($groups)) {
            $this->warn('No groups returned from Nissa API. Aborting.');
            return self::FAILURE;
        }
        ...
        $cachePath = storage_path('app/nissa_empty_branches_cache.json');
        $cachedEmptyBranches = [];
        ...
        // ------------------------------------------------------------------
        // 6. Pre-load existing Nissa vehicles from DB
        // ------------------------------------------------------------------
        $allExistingNissaVehicles = Vehicle::where('description', 'LIKE', '%[Nissa-GROUP-ID:%')
            ->get();

        $existingVehiclesByGroup = [];
        foreach ($allExistingNissaVehicles as $v) {
            if (preg_match('/\[Nissa-GROUP-ID:([^\]]+)\]/', $v->description, $m)) {
                $existingVehiclesByGroup[$m[1]][$v->pickup_loc] = $v;
            }
        }

        $this->info('Existing Nissa vehicles in DB: ' . $allExistingNissaVehicles->count());
        ...
        foreach ($groupsById as $groupId => $group) {
            ...
            try {
                ...
                if (empty($vehicleName)) {
                    $vehicleName = $groupName ?: 'Nissa Group ' . $groupId;
                }
                ...
                foreach ($branchesWithPrice as $branchId => $priceData) {
                    ...
                    if ($existingVehicle) {
                        ...
                        $vehicle = Vehicle::create([
                            'name' => $vehicleName,
                            'description' => '[Nissa-GROUP-ID:' . $groupId . '] ' . ($groupName ? "({$groupName})" : ''),
                            'photo' => $photoFilename,
                ...
                $missingBranchVehicles = Vehicle::where('description', 'LIKE', '%[Nissa-GROUP-ID:' . $groupId . ']%')
        ...
        // ------------------------------------------------------------------
        // 8. Delete orphaned Nissa vehicles no longer in any group
        // ------------------------------------------------------------------
        if (! $pricesOnly && ! empty($syncedVehicleIds)) {
            $orphaned = Vehicle::where('description', 'LIKE', '%[Nissa-GROUP-ID:%')
        ...
        $summaryPath = storage_path('app/nissa_sync_summary.txt');
        $summaryText = sprintf(
    ...
    /**
     * Sync vehicle specifications from Nissa group data.
     */
    ...
    private function downloadImage(?string $url, string $groupId): ?string
    {
        ...
        $filename = 'nissa_' . Str::slug($groupId) . '.' . $extension;
        ...
        try {
            ...
            if (! $response->successful()) {
                Log::warning("Nissa image download failed (HTTP {$response->status()})", ['url' => $url]);
            ...
            if (! str_starts_with($contentType, 'image/')) {
                Log::warning("Nissa resolved URL did not return an image", [
                    'url' => $url,
            ...
            Log::warning("Nissa image download exception", [
                'url' => $url,
                'error' => $e->getMessage(),
            ]);
            $this->imageCache[$cacheKey] = null;
