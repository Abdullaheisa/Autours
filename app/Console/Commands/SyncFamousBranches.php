...
class SyncFamousBranches extends Command
{
    protected $signature = 'famous:sync-branches {--dry-run : Only fetch from API and print what would be done} {--real : Perform the actual synchronization to the database}';

    protected $description = 'Sync Famous Rent A Car locations as branches.';
    ...
    public function handle(): int
    {
        ...
        $service = new KolaycarApiService('C0mMA5z64gqWPu5qs8MUjg==', 'G.!369niS.!76');

        $supplierUser = User::firstOrCreate(
            ['email' => 'info@famousrentacar.com'],
            [
                'name' => 'Famous Rent A Car',
                'role' => 'active_supplier',
                'password' => Hash::make('Qrentals@12345'),
                'company' => 'Famous Rent A Car',
            ]
        );
        $supplierUserId = $supplierUser->id;
        $this->info("Supplier user resolved: ID {$supplierUser->id} ({$supplierUser->email})");

        $this->info('Fetching locations from Famous Rent A Car...');
