...
class SyncXdriveBranches extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'xdrive:sync-branches {--dry-run : Only fetch from API and print what would be done} {--real : Perform the actual synchronization to the database}';
     ...
     * @var string
     */
    protected $description = 'Sync Xdrive Autotours locations as branches.';
    ...
    public function handle(): int
    {
        ...
        if ($this->option('dry-run')) {
            $this->info('[DRY RUN] Would resolve or create supplier user: xdrive@tempmail.com');
        } else {
        ...
        // ------------------------------------------------------------------
        // 2. Fetch all locations from Xdrive
        // ------------------------------------------------------------------
        $this->info('Fetching locations from Xdrive...');
        $locations = $service->getLocations();

        if (empty($locations)) {
            $this->error('No locations returned from Xdrive API.');
            return self::FAILURE;
        }
        ...
        // ------------------------------------------------------------------
        // 4. Delete branches no longer returned by Xdrive
        // ------------------------------------------------------------------
        ...
        $this->newLine();
        $this->info('========== Xdrive Branch Sync Complete ==========');
