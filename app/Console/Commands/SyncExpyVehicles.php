...
use App\Services\ExpyJsonApiService;
use Carbon\Carbon;
...
class SyncExpyVehicles extends Command
{
     ...
     * @var string
     */
    protected $signature = 'expy:sync-vehicles
     ...
     * @var string
     */
    protected $description = 'Sync vehicles from Expy JSON API into Expy. Creates one vehicle record per branch with per-station prices.';
    ...
    public function handle(): int
    {
        ...
        // ------------------------------------------------------------------
        // 1. Resolve or create the Expy supplier user
        // ------------------------------------------------------------------
        $supplierUserId = 0;
        if ($this->option('dry-run')) {
            $this->info('[DRY RUN] Would resolve or create supplier user: zehra@expycarrentals.com');
        } else {
            $supplierUser = User::firstOrCreate(
                ['email' => 'zehra@expycarrentals.com'],
                [
                    'name' => 'Expy',
            ...
            $supplierUser->update([
                'integration_type' => 'expy',
                'integration' => true,
            ]);

            $supplierUserId = $supplierUser->id;
            $this->info("Supplier user resolved: ID {$supplierUserId} ({$supplierUser->email})");
        }
        ...
        // ------------------------------------------------------------------
        // 3. Resolve all Expy branches
        // ------------------------------------------------------------------
        $allBranches = Branch::where('company_id', $supplierUserId)
        ...
        if ($allBranches->isEmpty()) {
            $this->warn('No Expy branches found. Run: php artisan expy:sync-branches');
            return self::FAILURE;
        }
        ...
        // ------------------------------------------------------------------
        // 4. Clean up orphaned Expy images
        // ------------------------------------------------------------------
        $this->info('Cleaning up orphaned Expy images...');
        $activeExpyPhotos = Vehicle::where('description', 'LIKE', '%[Expy-GROUP-ID:%')
            ->whereNotNull('photo')
            ->pluck('photo')
            ->unique()
            ->values()
        ...
        $cleaned = 0;
        if (is_dir($imgDir)) {
            foreach (glob($imgDir . '/expy_*') as $filePath) {
                $fileName = basename($filePath);
                if (! in_array($fileName, $activeExpyPhotos, true)) {
                    @unlink($filePath);
                    $cleaned++;
                }
            }
        ...
        // 4. Fetch all groups
        // ------------------------------------------------------------------
        $this->info('Fetching vehicle groups from Expy...');
        $groups = $service->getGroups();

        if (empty($groups)) {
            $this->warn('No groups returned from Expy API. Aborting.');
            return self::FAILURE;
        }
        ...
        $cachePath = storage_path('app/expy_empty_branches_cache.json');
        $cachedEmptyBranches = [];
        ...
        // ------------------------------------------------------------------
        // 6. Pre-load existing Expy vehicles from DB
        // ------------------------------------------------------------------
        $allExistingExpyVehicles = Vehicle::where('description', 'LIKE', '%[Expy-GROUP-ID:%')
            ->get();

        $existingVehiclesByGroup = [];
        foreach ($allExistingExpyVehicles as $v) {
            if (preg_match('/\[Expy-GROUP-ID:([^\]]+)\]/', $v->description, $m)) {
                $existingVehiclesByGroup[$m[1]][$v->pickup_loc] = $v;
            }
        }

        $this->info('Existing Expy vehicles in DB: ' . $allExistingExpyVehicles->count());
        ...
        foreach ($groupsById as $groupId => $group) {
            ...
            try {
                ...
                if (empty($vehicleName)) {
                    $vehicleName = $groupName ?: 'Expy Group ' . $groupId;
                }
                ...
                foreach ($branchesWithPrice as $branchId => $priceData) {
                    ...
                    if ($existingVehicle) {
                        ...
                        $vehicle = Vehicle::create([
                            'name' => $vehicleName,
                            'description' => '[Expy-GROUP-ID:' . $groupId . '] ' . ($groupName ? "({$groupName})" : ''),
                            'photo' => $photoFilename,
                ...
                $missingBranchVehicles = Vehicle::where('description', 'LIKE', '%[Expy-GROUP-ID:' . $groupId . ']%')
        ...
        // ------------------------------------------------------------------
        // 8. Delete orphaned Expy vehicles no longer in any group
        // ------------------------------------------------------------------
        if (! $pricesOnly && ! empty($syncedVehicleIds)) {
            $orphaned = Vehicle::where('description', 'LIKE', '%[Expy-GROUP-ID:%')
        ...
        if ($created > 0) {
            try {
                \Illuminate\Support\Facades\Mail::raw(
                    "Automated Sync Alert: {$created} new vehicle(s) have been added from Expy Supplier. Standard 5% profit margin assigned.",
                    function ($message) use ($created) {
                        $message->to(['admin@autours.net', 'contact@autours.net'])
                                ->subject("Expy Sync Notification: {$created} New Vehicle(s) Added");
                    }
                );
            } catch (\Exception $e) {
                Log::error("Failed to send Expy new vehicle notification email: " . $e->getMessage());
            }
        }

        $this->newLine();
        $this->info('========== Expy Vehicle Sync Complete ==========');
        ...
        $summaryPath = storage_path('app/expy_sync_summary.txt');
        $summaryText = sprintf(
    ...
    private function syncInclusions(Vehicle $vehicle, array $priceData): void
    {
        ...
        $includes = [
            'cdw_free' => 'Collision Damage Waiver (CDW)',
            'scdw_free' => 'Super Collision Damage Waiver (SCDW)',
            'lcf_free' => 'Liability Coverage (LCF)',
            'pai_free' => 'Personal Accident Insurance (PAI)',
        ];

        foreach ($includes as $key => $text) {
            if (!empty($priceData[$key])) {
                $inc = \App\Models\Included::firstOrCreate(['what_is_included' => $text]);
                $includedIds[] = $inc->id;
            }
        }
    ...
    /**
     * Sync vehicle specifications from Expy group data.
     */
    ...
    private function downloadImage(?string $url, string $groupId): ?string
    {
        ...
        $filename = 'expy_' . Str::slug($groupId) . '.' . $extension;
        ...
        try {
            ...
            if (! $response->successful()) {
                Log::warning("Expy image download failed (HTTP {$response->status()})", ['url' => $url]);
            ...
            if (! str_starts_with($contentType, 'image/')) {
                Log::warning("Expy resolved URL did not return an image", [
                    'url' => $url,
            ...
            Log::warning("Expy image download exception", [
                'url' => $url,
                'error' => $e->getMessage(),
            ]);
            $this->imageCache[$cacheKey] = null;
