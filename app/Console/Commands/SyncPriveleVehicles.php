...
class SyncPriveleVehicles extends Command
{
     ...
     * @var string
     */
    protected $signature = 'privele:sync-vehicles
     ...
     * @var string
     */
    protected $description = 'Sync vehicles from Privele Mobility API into Autours. Creates one vehicle record per group per rate code per branch.';
    ...
    public function handle(): int
    {
        ...
        // ------------------------------------------------------------------
        // 3. Resolve all Privele branches
        // ------------------------------------------------------------------
        ...
        if ($allBranches->isEmpty()) {
            $this->warn('No Privele branches found. Run: php artisan privele:sync-branches');
            return self::FAILURE;
        }
        ...
        // ------------------------------------------------------------------
        // 6. Pre-load existing Privele vehicles from DB
        // ------------------------------------------------------------------
        $allExistingSurpriceVehicles = Vehicle::where('description', 'LIKE', '%[PRIVELE-GROUP-ID:%')
            ->get();

        $existingVehiclesByTag = [];
        foreach ($allExistingSurpriceVehicles as $v) {
            if (preg_match('/\[PRIVELE-GROUP-ID:([^\]]+)\]/', $v->description, $m)) {
                $existingVehiclesByTag[$m[1]][$v->pickup_loc] = $v;
            }
        }

        $this->info('Existing Privele vehicles in DB: ' . $allExistingSurpriceVehicles->count());
        ...
        foreach ($rateCodes as $rateCode) {
            ...
            foreach ($stationToBranchMap as $stationId => $branchId) {
                ...
                foreach ($prices as $groupId => $priceData) {
                    $tag = "[PRIVELE-GROUP-ID:{$groupId}|RATE:{$rateCode}]";
        ...
        $missingBranchVehicles = Vehicle::where('description', 'LIKE', '%[PRIVELE-GROUP-ID:%')
        ...
        if ($inactiveBranchIds->isNotEmpty()) {
            $inactiveVehicles = Vehicle::where('description', 'LIKE', '%[PRIVELE-GROUP-ID:%')
        ...
        // ------------------------------------------------------------------
        // 10. Delete orphaned Privele vehicles no longer in any group
        // ------------------------------------------------------------------
        if (! $pricesOnly && ! empty($syncedVehicleIds)) {
            $orphaned = Vehicle::where('description', 'LIKE', '%[PRIVELE-GROUP-ID:%')
        ...
        if ($created > 0) {
            try {
                \Illuminate\Support\Facades\Mail::raw(
                    "Automated Sync Alert: {$created} new vehicle(s) have been added from Privele Supplier. Standard 5% profit margin assigned.",
                    function ($message) use ($created) {
                        $message->to(['admin@autours.net', 'contact@autours.net'])
                                ->subject("Privele Sync Notification: {$created} New Vehicle(s) Added");
                    }
                );
            } catch (\Exception $e) {
                Log::error("Failed to send Privele new vehicle notification email: " . $e->getMessage());
            }
        }

        $this->newLine();
        $this->info('========== Privele Vehicle Sync Complete ==========');
    ...
    /**
     * Sync vehicle specifications from Privele vehicle data.
     */
    ...
    private function downloadImage(?string $url, string $groupId): ?string
    {
        ...
        $filename = 'privele_' . Str::slug($groupId) . '.' . $extension;
        ...
        try {
            ...
            if (! $response->successful()) {
                Log::warning("Privele image download failed (HTTP {$response->status()})", ['url' => $url]);
            ...
            if (! str_starts_with($contentType, 'image/')) {
                Log::warning("Privele resolved URL did not return an image", [
                    'url' => $url,
            ...
            Log::warning("Privele image download exception", [
                'url' => $url,
                'error' => $e->getMessage(),
            ]);
            $this->imageCache[$cacheKey] = null;
