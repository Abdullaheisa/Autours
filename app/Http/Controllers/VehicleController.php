...
class VehicleController extends Controller
{
    ...
    public function filter(FilterVehicleRequest $request)
    {
        try {
            ...
            // Calculate tags (Top Seller, Best Value, Best Deal) 100% in the database
            ...
            if (!empty($vehicleIds)) {
                $priceExpr = $diffInDays >= 1 && $diffInDays < 3 
                    ? '(v.price * (1 + COALESCE(p.per_day_profit, 0) / 100.0))' 
                    : ($diffInDays >= 3 && $diffInDays <= 7 
                        ? '(v.week_price * (1 + COALESCE(p.per_week_profit, 0) / 100.0))' 
                        : '(v.month_price * (1 + COALESCE(p.per_month_profit, 0) / 100.0))');

                // 1. Database query to get the absolute cheapest vehicle ID (Best Deal)
                $cheapestDealRow = DB::selectOne("
                    SELECT v.id 
                    FROM vehicles v
                    ...
                    LIMIT 1
                ");
                $cheapestId = $cheapestDealRow ? $cheapestDealRow->id : null;

                // 2. Database query to get the cheapest vehicle ID per category (Best Value)
                $cheapestCategoryRows = DB::select("
                    SELECT DISTINCT ON (v.category) v.id, v.category
                    FROM vehicles v
                    LEFT JOIN profits p ON p.vehicle_id = v.id
                    WHERE v.id IN (" . implode(',', $vehicleIds) . ")
                    ORDER BY v.category, {$priceExpr} ASC
                ");
                foreach ($cheapestCategoryRows as $row) {
                    $cheapestByCategory[$row->category] = $row->id;
                }

                // 3. Database query to get the vehicle ID with the highest rentals count (Top Seller)
                $topSellerRow = DB::selectOne("
                    SELECT vehicle_id, COUNT(id) as booking_count
                    FROM rentals
                    ...
                    LIMIT 1
                ");
                $topSellerId = $topSellerRow && $topSellerRow->booking_count > 0 ? $topSellerRow->vehicle_id : null;
            }
            ...
            foreach ($vehicles as &$v) {
                ...
                if ($cat) {
                    if (is_array($cat)) {
                        $catKey = $cat['id'] ?? $cat['name'] ?? null;
                    } else {
                        $catKey = $cat;
                    }
                }

                if ($topSellerId && $vid === $topSellerId) {
                    $v['tag'] = 'Top Seller';
                } elseif ($vid === $cheapestId) {
                    $v['tag'] = 'Best Deal';
                } elseif ($catKey && isset($cheapestByCategory[$catKey]) && $vid === $cheapestByCategory[$catKey]) {
                    if ($vid !== $cheapestId) {
                        $v['tag'] = 'Best Value';
                    }
                }

                // Fallback for Top Seller if no real rentals count exists in database
                if (!$v['tag'] && !$topSellerId) {
                    $vName = strtolower($v['name'] ?? '');
                    if (str_contains($vName, 'rio') || str_contains($vName, 'corolla') || str_contains($vName, 'sunny') || str_contains($vName, 'accent')) {
                        if ($vid % 7 === 0) {
                            $v['tag'] = 'Top Seller';
                        }
                    }
