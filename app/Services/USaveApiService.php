...
class USaveApiService
{
    private const BASE_URL = 'https://sv.fusemetrix.com/bespoke/SVWebService.php';
    ...
    /**
     * Send an XML request to the U-Save API.
     */
    private function sendRequest(string $method, array $params = []): array
    {
        ...
        if (!$response->successful()) {
            Log::error("U-Save API: {$method} request failed", [
    ...
    private function parseResponse(string $xmlContent): array
    {
        try {
            ...
            if ($xml === false) {
                ...
                Log::error('U-Save API: Failed to parse XML response', [
                    'errors' => $errors,
                    'xml' => $xmlContent
                ]);
                libxml_clear_errors();
            ...
            if (isset($xml->response->errors)) {
                ...
                Log::error("U-Save API returned error: [{$errorCode}] {$errorMsg}");
                throw new \Exception("U-Save API error: [{$errorCode}] {$errorMsg}");
            }
            ...
            Log::error('U-Save API: Exception parsing response', ['error' => $e->getMessage()]);
            throw $e;
        }
    }
