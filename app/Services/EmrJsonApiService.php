...
class EmrJsonApiService
{
    private const BASE_URL = 'http://emrjson.turevrent.com/';
    private const KEY_HACK = '9498a4e2-ac32-4af4-b4ef-e9b86035bf9c';
    private const USERNAME = 'EmrAUTOURS';
    private const PASSWORD = 'iteno6gzskk84hb';
    ...
    public function getLocations(): array
    {
        ...
        if (! $response->successful()) {
            Log::error('EMR API: Failed to fetch locations', [
    ...
    public function getGroups(): array
    {
        ...
        if (! $response->successful()) {
            Log::error('EMR API: Failed to fetch groups', [
                'status' => $response->status(),
                'body' => $response->body(),
            ]);
            return [];
    ...
    public function getAvailableCars(
        ...
        if (! $response->successful()) {
            Log::error('EMR API: Failed to fetch available cars', [
                'status' => $response->status(),
                'pickup_id' => $pickupId,
    ...
    public function getAvailableCarsForStations(
        ...
        foreach (array_chunk($stationIds, $concurrency) as $chunk) {
            try {
                ...
                Log::error('EMR API: Concurrent price fetch failed', ['error' => $e->getMessage()]);
                continue;
            }
    ...
    public function saveReservation(array $params): array
    {
        ...
        if (! $response->successful()) {
            Log::error('EMR API: Failed to save reservation', [
                'status' => $response->status(),
            ]);
            return [];
        }
    ...
    public function cancelReservation(string $rezId, string $id): array
    {
        ...
        if (! $response->successful()) {
            Log::error('EMR API: Failed to cancel reservation', [
                'status' => $response->status(),
            ]);
            return [];
        }
    ...
    private function parseResponse($response): array
    {
        ...
        if (! is_array($data)) {
            Log::warning('EMR API: Unexpected response format', [
