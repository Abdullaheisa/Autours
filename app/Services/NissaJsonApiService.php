...
class NissaJsonApiService
{
    private const BASE_URL = 'http://nissajson.nissaxml.com/';
    private const KEY_HACK = '0a54ba75-23b1-4ac2-a6fa-4fa4ac6481cd';
    private const USERNAME = '253Nissa2tAutooursfmj';
    private const PASSWORD = 'ebu2ulk6209tmvd';

    private int $requestTimeout = 15;

    /**
     * Fetch all locations from Nissa JSON API.
     *
    ...
    public function getLocations(): array
    {
        ...
        if (! $response->successful()) {
            Log::error('Nissa API: Failed to fetch locations', [
    ...
    /**
     * Fetch all vehicle groups from Nissa JSON API.
     *
     * @return array<int, array>
     */
    public function getGroups(): array
    {
        ...
        if (! $response->successful()) {
            Log::error('Nissa API: Failed to fetch groups', [
                'status' => $response->status(),
                'body' => $response->body(),
            ]);
            return [];
    ...
    public function getAvailableCars(
        ...
        if (! $response->successful()) {
            Log::error('Nissa API: Failed to fetch available cars', [
                'status' => $response->status(),
                'pickup_id' => $pickupId,
    ...
    public function getAvailableCarsForStations(
        ...
        foreach (array_chunk($stationIds, $concurrency) as $chunk) {
            try {
                ...
                Log::error('Nissa API: Concurrent price fetch failed', ['error' => $e->getMessage()]);
                continue;
            }
    ...
    public function saveReservation(array $params): array
    {
        ...
        if (! $response->successful()) {
            Log::error('Nissa API: Failed to save reservation', [
                'status' => $response->status(),
            ]);
            return [];
        }
    ...
    public function cancelReservation(string $rezId, string $id): array
    {
        ...
        if (! $response->successful()) {
            Log::error('Nissa API: Failed to cancel reservation', [
                'status' => $response->status(),
            ]);
            return [];
        }
    ...
    private function parseResponse($response): array
    {
        ...
        if (! is_array($data)) {
            Log::warning('Nissa API: Unexpected response format', [
