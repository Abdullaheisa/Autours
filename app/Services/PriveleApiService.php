...
class PriveleApiService
{
    private const BASE_URL = 'https://web-service.privellerentals.com';
    private const BEARER_TOKEN = '48916be2-1dcf-49fd-aadd-37cb4cfb5a03';

    private int $requestTimeout = 30;

    /**
     * Fetch all cities from Privele API.
     *
    ...
    public function getCities(): array
    {
        ...
        if (! $response->successful()) {
            Log::error('Privele API: Failed to fetch cities', [
    ...
    /**
     * Fetch all countries from Privele API.
     *
     * @return array<int, array>
     */
    public function getCountries(): array
    {
        ...
        if (! $response->successful()) {
            Log::error('Privele API: Failed to fetch countries', [
                'status' => $response->status(),
                'body' => $response->body(),
            ]);
            return [];
    ...
    /**
     * Fetch all rental locations/stations from Privele API.
     *
     * @return array<int, array>
     */
    public function getLocations(int $limit = 500): array
    {
        ...
        if (! $response->successful()) {
            Log::error('Privele API: Failed to fetch locations', [
                'status' => $response->status(),
                'body' => $response->body(),
            ]);
            return [];
    ...
    public function getAvailability(
        ...
        if (! $response->successful()) {
            Log::error('Privele API: Availability request failed', [
                'status' => $response->status(),
    ...
    public function getAvailabilityForStations(
        ...
        foreach (array_chunk($stationIds, $concurrency) as $chunk) {
            try {
                ...
                Log::error('Privele API: Concurrent availability fetch failed', ['error' => $e->getMessage()]);
                continue;
            }
    ...
    /**
     * Create a reservation on Privele.
     *
    ...
    public function createReservation(array $params): array
    {
        ...
        if (! $response->successful()) {
            ...
            Log::error('Privele API: Failed to create reservation', [
                'status' => $status,
                'body' => $response->body(),
            ]);
    ...
    public function amendReservation(string $orderId, array $params): array
    {
        ...
        if (! $response->successful()) {
            Log::error('Privele API: Failed to amend reservation', [
                'status' => $response->status(),
                'order_id' => $orderId,
                'body' => $response->body(),
            ]);
    ...
    public function commitReservation(string $orderId): array
    {
        ...
        if (! $response->successful()) {
            Log::error('Privele API: Failed to commit reservation', [
                'status' => $response->status(),
                'order_id' => $orderId,
                'body' => $response->body(),
            ]);
    ...
    public function ignoreReservation(string $orderId): array
    {
        ...
        if (! $response->successful()) {
            Log::error('Privele API: Failed to ignore reservation amendment', [
                'status' => $response->status(),
                'order_id' => $orderId,
                'body' => $response->body(),
            ]);
