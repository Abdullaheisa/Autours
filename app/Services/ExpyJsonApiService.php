...
class ExpyJsonApiService
{
    private const BASE_URL = 'https://expycarjson.trvrac.com/';
    private const KEY_HACK = '46fa7b91-b07c-4a8c-8525-dae7cff26864';
    private const USERNAME = 'expyautorbmhnpn';
    private const PASSWORD = 'htdp0b8f3z9zb0o';

    private int $requestTimeout = 15;

    /**
     * Fetch all locations from Expy JSON API.
     *
    ...
    public function getLocations(): array
    {
        ...
        if (! $response->successful()) {
            Log::error('Expy API: Failed to fetch locations', [
    ...
    /**
     * Fetch all vehicle groups from Expy JSON API.
     *
     * @return array<int, array>
     */
    public function getGroups(): array
    {
        ...
        if (! $response->successful()) {
            Log::error('Expy API: Failed to fetch groups', [
                'status' => $response->status(),
                'body' => $response->body(),
            ]);
            return [];
    ...
    public function getAvailableCars(
        ...
        if (! $response->successful()) {
            Log::error('Expy API: Failed to fetch available cars', [
                'status' => $response->status(),
                'pickup_id' => $pickupId,
    ...
    public function getAvailableCarsForStations(
        ...
        foreach (array_chunk($stationIds, $concurrency) as $chunk) {
            try {
                ...
                Log::error('Expy API: Concurrent price fetch failed', ['error' => $e->getMessage()]);
                continue;
            }
    ...
    public function saveReservation(array $params): array
    {
        ...
        if (! $response->successful()) {
            Log::error('Expy API: Failed to save reservation', [
                'status' => $response->status(),
            ]);
            return [];
        }
    ...
    public function cancelReservation(string $rezId, string $id): array
    {
        ...
        if (! $response->successful()) {
            Log::error('Expy API: Failed to cancel reservation', [
                'status' => $response->status(),
            ]);
            return [];
        }
    ...
    private function parseResponse($response): array
    {
        ...
        if (! is_array($data)) {
            Log::warning('Expy API: Unexpected response format', [
    ...
    private function deduplicateCarsByGroup(array $cars): array
    {
        ...
        foreach ($cars as $car) {
            ...
            $prices[$gid] = [
                ...
                'cdw_free' => filter_var($car['CDW_Free'] ?? $car['cdw_free'] ?? false, FILTER_VALIDATE_BOOLEAN),
                'scdw_free' => filter_var($car['SCDW_Free'] ?? $car['scdw_free'] ?? false, FILTER_VALIDATE_BOOLEAN),
                'lcf_free' => filter_var($car['LCF_Free'] ?? $car['lcf_free'] ?? false, FILTER_VALIDATE_BOOLEAN),
                'pai_free' => filter_var($car['PAI_Free'] ?? $car['pai_free'] ?? false, FILTER_VALIDATE_BOOLEAN),
            ];
        }
