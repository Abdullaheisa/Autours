...
function SearchPageContent() {
  ...
  // Use refs so callbacks always read the latest values without stale closures
  ...
  const buildFilterPayload = useCallback((): FilterPayload | null => {
    ...
    if (fp.priceRange) payload.priceRange = fp.priceRange[1];
    // Convert string[] → number[] for API
    if (fp.category.length > 0) payload.category = fp.category.map(Number);
    if (fp.supplier.length > 0) payload.supplier = fp.supplier.map(Number);
    if (fp.locationType.length > 0) payload.location_type_id = fp.locationType.map(Number);
    if (fp.paymentType.length > 0) payload.payment_methods = fp.paymentType.map(Number);

    // Group specifications
    const specifications: { name: string; option: string[] }[] = [];
    if (fp.seats.length > 0) specifications.push({ name: 'seats', option: fp.seats });
    if (fp.doors.length > 0) specifications.push({ name: 'doors', option: fp.doors });
    if (fp.transmission.length > 0) specifications.push({ name: 'transmission', option: fp.transmission });
    if (fp.fuelType.length > 0) specifications.push({ name: 'fuel_type', option: fp.fuelType });
    if (fp.suitcases.length > 0) specifications.push({ name: 'suitcases', option: fp.suitcases });
    if (fp.airConditioning) specifications.push({ name: 'ac', option: [fp.airConditioning] });

    if (specifications.length > 0) payload.specifications = specifications;

    if (fp.rating !== null) payload.rating = fp.rating;
    if (fp.sortBy) payload.sortBy = fp.sortBy;

    return payload;
  }, []); // stable — always reads latest via refs
  ...
  // Trigger on initial search params or currency change
  useEffect(() => {
  ...
  // Trigger on any filter change — refs ensure we read the latest filterParams
  ...
  // kept for components that call it directly (no-op since effect handles it)
  const handleFilterChange = useCallback(() => {
    // intentionally empty — filterParams useEffect above handles re-fetch
  }, []);
