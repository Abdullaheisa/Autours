...
import { formatPrice, convertFromUsd } from '@/utils/currency';
...
export default function CarCard({ vehicle, daysNumber, hideBookingControls = false }: CarCardProps) {
  ...
  const { code: currencyCode, rate, allRates } = currencyState;
  ...
  const carData = useMemo(() => {
    ...
    // price_in_usd is always the base; convert to selected currency
    const priceInUsd = vehicle.price_in_usd || (vehicle as any).price || 0;
    const totalPrice = convertFromUsd(priceInUsd, currencyCode as any, allRates);
