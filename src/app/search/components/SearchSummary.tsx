...
import { CheckCircle2, Calendar, Clock, Pencil } from 'lucide-react';
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm">
      ...
      <div className={`${forceMobileLayout ? 'block' : 'md:hidden'} relative`}>
        <div className={`flex flex-col sm:flex-row sm:items-center bg-gray-100 ${!hideEditButton ? 'pr-12' : ''}`}>
          {/* Pickup Info */}
          <div className="flex-1 px-4 py-3 sm:border-r border-gray-200">
          ...
          {/* Return Info */}
          <div className="flex-1 px-4 py-3 border-t sm:border-t-0 border-gray-200">
            <p className="text-xs font-bold text-gray-800 truncate">
              {searchParams.location || 'Select Location'}
            </p>
            <p className="text-[10px] text-gray-500 mt-0.5">
        ...
        {!hideEditButton && (
          <button
            onClick={onEditClick}
            className="absolute right-0 top-0 bottom-0 w-12 flex items-center justify-center bg-gray-200 text-gray-600 hover:bg-primary hover:text-gray-900 transition-colors"
